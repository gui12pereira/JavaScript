...
//Resgatando os IDs
$sigla = document.getElementById("txt_sigla");
...
const mostrarCidades = () => {
   ...
   //Limpando as cidades para não ter sobrecarga
   $cidades.innerHTML= "";

   //Filtro que seleciona todas as cidades do estado escrito pela sigla
   const estadoSelecionado = cidades.filter( ( elemento ) => elemento.sigla == $sigla.value.toUpperCase() );

   //Por padrão, a primeira cidade sempre tem o tema claro
   let tema = "";

   //Estrutura de repetição que, a partir da variável de controle, enumera todas as cidades do estado requerido
   for( i = 1; i <= estadoSelecionado[0].cidades.length; i++ ) {
      $cidades.innerHTML += `
      <div class="identificacao ${tema}">
         ${i}
      </div>
      <div class="listagem ${tema}">
         ${estadoSelecionado[0].cidades[i-1].nome}
      </div>
      `;

      //Estruta de decisão que faz a alternância das cores
      if (tema == "")
         tema = "escuro";
      else
         tema = "";
